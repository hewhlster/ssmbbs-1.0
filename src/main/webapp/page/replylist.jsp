		...
		<c:forEach items="${topicmap}" var="topic">
			...
			<div class="row" style="border: solid orange 1px;">
				...
				<div
					...
					<div >
						<img style="uhead-img" src="<%=basePath %>img/head/${topic.key.tuser.uhead}" alt="" class="topic-left-user-head" />
					</div>
					<div>${topic.key.tuser.uname} | 帖子数:<span>100</span></div>
					<div></div>
					
					<div>查看:${topic.key.treaded} | 回复：${topic.key.tgooded}</div>
				</div>
				<!--
					右侧帖子信息
				-->
			...
			<c:forEach items="${ topic.value }" var="reply" varStatus="vs">
                ...
                -->
					<div
						...
						<div style="height: 30px;"></div>
												<div class="uhead">
							<img src="<%=basePath %>img/head/${reply.ruser.uhead}" alt="" class="topic-left-user-head" />
						</div>
						<div>${reply.ruser.uname}</div>
...
<script type="text/javascript" src="<%=basePath%>/bt/js/bootstrap.js"></script>
